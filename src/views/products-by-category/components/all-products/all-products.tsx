...
const AllProducts = ({ loading, propsductsCollection, expand, singleProductsEmit = () => { } }: props) => {
    ...
    return (
        <>
            {!propsductsCollection.length && !loading
                ? <div className="no-items"> <span> No Items Found</span> </div>
                : <div className={`content-all-products ${!expand ? "expand" : ""}`}>
                    {loading
                        ? itemsToLoad.map(() => <div className="wave"></div>)
                        : propsductsCollection.map(p =>
